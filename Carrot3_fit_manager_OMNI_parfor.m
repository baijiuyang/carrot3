close all;
clear all;
load Carrot3_data_piped(4th1Hz)
format shortg;
startTime = tic;
modelList = {
    'nullModel';
...
method = 'LOSO'; % MC: Monte Carlo
                % LOO: leave-one-out (leave one trial in a condition of a subject out)
                % ALL: fit to all data
...
if strcmp(method,'LOSO')
    nIter = max([following.subject]);
end
simLength = 6; % 6 seconds of data in each trial is used for training and testing
...
%% choose data set
following = rmfield(following, {'raw_traj';'zero_traj';'rot_traj';'inter_traj';'filtered_traj'});
DATA_SET = following(1);

for trial = following
    if trial.dump == 0 && trial.dv ~= 0 && trial.t_total-trial.manipOnset >= simLength
...
if strcmp(method,'LOO')
    ...
    for subject = 1:13
        for d0 = [1 4 8]
            for v0 = [0.8 1.2]
                for dv = [-0.3 0.3]
                    PoolList(end+1).subjCon = ['subj' num2str(subject) '_' num2str(d0) num2str(v0) num2str(dv)];
                    for i = 1:length(DATA_SET)
                        if DATA_SET(i).subject == subject && DATA_SET(i).d0 == d0 &&...
                                DATA_SET(i).v0 == v0 && DATA_SET(i).dv == dv
...
parfor ii = 1:size(indexList,2)
    ...
    if ~strcmp(model,'nullModel')
        zFunction = @(p) train(model, p, delay, TRAIN_SET, criterion, variable, simLength);
    ...
    TEST_SET = test(TEST_SET, model, iIter, pIter, delay, simLength);
    ...
    ResultsSummary(ii).testError = mean([TEST_SET.RMSE_v]);
    Results_ = [Results_, TEST_SET];
...
% write results
save(['Carrot3_fit_pertTrials_',method,num2str(nIter),'_',criterion,'_',variable,'_',char(join(time(1:5),'-')),'.mat'],...
...
function index = train(model, p, delay, TRAIN_SET, criterion, variable, simLength)
    Hz = 60;
    ...
    for i = 1:length(TRAIN_SET)
        ...
        tEnd = tStart + simLength*Hz - 1;

        lPos = TRAIN_SET(i).data(tStart:tEnd,1);
        lSpd = TRAIN_SET(i).data(tStart:tEnd,3);
        fPos = TRAIN_SET(i).data(tStart:tEnd,2);
        fSpd = TRAIN_SET(i).data(tStart:tEnd,4);
        fAcc = TRAIN_SET(i).data(tStart:tEnd,6);
        inputHz = 60;
        outputHz = 60;
        pStart = fPos(1);
        vStart = fSpd(1);
...
function TEST_SET = test(TEST_SET, model, iIter, p, delay, simLength)
    Hz = 60;  
    
    for i = 1:length(TEST_SET)
            manipOnset = TEST_SET(i).manipOnset;
            tStart = int32((manipOnset-0.5)*Hz)+1;
            tEnd = tStart + simLength*Hz - 1;
            lPos = TEST_SET(i).data(tStart:tEnd,1);
            lSpd = TEST_SET(i).data(tStart:tEnd,3);
            fPos = TEST_SET(i).data(tStart:tEnd,2);
            fSpd = TEST_SET(i).data(tStart:tEnd,4);
            fAcc = TEST_SET(i).data(tStart:tEnd,6);
            inputHz = 60;
            outputHz = 60;
            pStart = fPos(1);
            vStart = fSpd(1);
            ...
            TEST_SET(i).tests(:,1) = fPos;
            TEST_SET(i).tests(:,2) = mPos;
            TEST_SET(i).tests(:,3) = fSpd;
            TEST_SET(i).tests(:,4) = mSpd;
            TEST_SET(i).tests(:,5) = fAcc;
            TEST_SET(i).tests(:,6) = mAcc;
            TEST_SET(i).z_x = atanh(corr(fPos, mPos, 'type','pearson'));
            TEST_SET(i).z_v = atanh(corr(fSpd, mSpd, 'type','pearson'));
            TEST_SET(i).z_a = atanh(corr(fAcc, mAcc, 'type','pearson'));           
            TEST_SET(i).RMSE_x = sqrt(mean((fPos - mPos).^2));
            TEST_SET(i).RMSE_v = sqrt(mean((fSpd - mSpd).^2));
            TEST_SET(i).RMSE_a = sqrt(mean((fAcc - mAcc).^2));
            TEST_SET(i).model = model;
            TEST_SET(i).iIter = iIter;
    end
end
